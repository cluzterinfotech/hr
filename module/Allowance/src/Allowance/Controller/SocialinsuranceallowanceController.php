...
class SocialinsuranceallowanceController extends AbstractActionController {
	
	private $tableName = 'SocialInsuranceAllowance';
	...
	public function ajaxlistAction() {
		$grid = $this->getSiAllowanceGrid(); 
		$grid->setAdapter($this->getDbAdapter())
		     ->setSource($this->getCompanyAllowanceService()
		     		          ->selectSocialInsuranceAllowance())
	...
	public function addAction() {
		...
		$prg = $this->prg('/siallowance/add', true);
		...
		if ($form->isValid()) {
			...
			$this->flashMessenger()->setNamespace('success')
			                       ->addMessage('Social Insurance Allowance added successfully'); 
	...
	public function editAction() {
		...
		if (!$id) {
			$this->flashMessenger()->setNamespace('info')
			                       ->addMessage('Social Insurance Allowance not found,Please Add');
			$this->redirect()->toRoute('siallowance', array(
					'action' => 'add'
			));  
		} 
		...
		$prg = $this->prg('/siallowance/edit/'.$id, true);
		if ($prg instanceof Response ) {
			return $prg;
		} elseif ($prg === false) {
			return array ('form' => $form);
	...
	private function getSiAllowanceGrid() {
		return new SiAllowanceGrid();
	}
