...
use Application\Model\AffiliationAmountGrid;

class AffiliationamountController extends AbstractActionController {
	...
	public function ajaxlistAction() {
		...
		$grid->setAdapter($this->getDbAdapter())
		     ->setSource($this->getAffiliationService()->selectAffiliationAmount()) 
	...
	public function applynewAction() { 
		...
		$form->setData($prg); 
		//$salaryGradeService = $this->getAffiliationService(); 
	...
	public function updateexistingAction() { 
		...
		$service = $this->getAffiliationService(); 
	...
	public function removeAction()
	{
		// @todo 
		$id = (int) $this->params()->fromRoute('id',0);
		//$sgService = $this->getAffiliationService();
		//echo json_encode($id); 
		$this->getAffiliationService()->removeSgAllowanceBuffer($id);
		exit; 
	}
	
	protected function savesgallowanceAction() {
		$formValues = $this->params()->fromPost('formVal');
		//$employeeService = $this->getAffiliationService();
		//var_dump($formValues); 
		//exit; 
		$company = $this->getServiceLocator()->get('company'); 
		$this->getAffiliationService()->saveSgAllowanceBuffer($formValues,$company); 
		exit; 
	} 
	...
	private function getAffiliationService() {
		return $this->getServiceLocator()->get('affiliationService');
	}
	
	private function getNotConfirmedEmployeeList() {
		return $this->getAffiliationService()->notConfirmedEmployeeList();
	}
	...
	public function getSalaryGradeList() {
		...
		return $this->getAffiliationService()->getSalaryGradeList();  
	}
	...
	private function getExistingGrid() {
		return new AffiliationAmountGrid(); 
	} 
