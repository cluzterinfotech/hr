...
use Employee\Form\AllowanceSpecialAmountForm;
use Application\Model\EmployeeSpecialAmountGrid;
use Application\Form\SubmitButonForm;
use Employee\Form\DeleteAllowanceSpecialAmountForm;

class AllowancespecialamountController extends AbstractActionController {
    
	public function ajaxlistAction() {
		$grid = $this->getGrid();
		$grid->setAdapter($this->getDbAdapter())
		     ->setSource($this->getEmployeeService()
		     ->selectSpecialAmountBuffer())  
	...
	{   
		$form = new AllowanceSpecialAmountForm();
		$form->get('employeeNumberAllowance') 
		     ->setOptions(array('value_options' => $this->getEmployeeList())) 
		;    
		$form->get('isAdded')->setValue('1');
	...
	public function deletespecialamountAction() {
	    $form =  new AllowanceSpecialAmountForm();
	    $form->get('employeeNumberAllowance')
	    ->setOptions(array('value_options' => $this->getEmployeeList()))
	    ;
	    $form->get('isAdded')->setValue('0');
	    $form->get('allowanceId')
	    ->setOptions(array('value_options' => $this->getAllowanceList()))
	...
	public function applyspecialamountAction() { 
	     
		$form = $this->getForm();
		$form->get('submit')->setValue('Apply Special Amount');
		$prg = $this->prg('/allowancespecialamount/applyspecialamount', true); 
		...
		if ($form->isValid()) {  
			...
			$response = $employeeService->applySpecialAmount($company); 
			$this->flashMessenger()->setNamespace('info')
			     ->addMessage($response);   
			$this->redirect()->toRoute('allowancespecialamount',array(
				'action' => 'list' 
			));   
		} 
		return array( 
	...
	{   
		...
		$this->getEmployeeService()->removeEmployeeSpecialAmountBuffer($id); 
		exit;  
	} 
 
    protected function saveemployeeSpecialAmountAction() {
		$formValues = $this->params()->fromPost('formVal');   
		$this->getEmployeeService()->saveEmployeeAllowanceSpecialAmount($formValues);   
		exit; 
	}    
	...
	public function getoldallowanceAction() { 
		$employeeNumber = $this->params()->fromPost('empNumber'); 
		$allowanceId = $this->params()->fromPost('allowId');
		
		$employeeService = $this->getEmployeeService();
		$amount = $employeeService->getEmployeeAllowanceAmount($employeeNumber , $allowanceId);
	...
	private function getGrid() { 
		return new EmployeeSpecialAmountGrid();
	}
