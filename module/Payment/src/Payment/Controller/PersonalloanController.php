...
class PersonalloanController extends AbstractActionController {
	...
	public function ajaxlistAction() {
		...
		$grid->setAdapter($this->getDbAdapter())
		     ->setSource($this->getAdvancePaymentService()->selectPersonalLoan())
	...
	{   
		...
		$form = new PersonalLoanForm(); // AdvanceSalaryForm(); 
		$form->get('employeeNumberPersonalLoan') 
	...
	public function removeAction()
	{
		...
		$advancePaymentService->removePersonalLoan($id,$routeInfo); 
		//echo json_encode($id); 
		exit; 
	}
	...
	private function getForm() {
        ...
        $form->get('submit')->setValue('Close Personal Loan');  
		return $form;  
	}  
	
	public function closeAction() { 
		$form = $this->getForm();
		$prg = $this->prg('/personalloan/close', true);
		if ($prg instanceof Response ) {
			return $prg;
		} elseif ($prg === false) {
			return array ('form' => $form);
		...
		//} else { 
			$routeInfo = $this->getRouteInfo(); 
			$res = $service->closePersonalLoan($routeInfo);     
			$this->flashMessenger()->setNamespace('success') 
			     ->addMessage($res); 
		//} 
		$this->redirect()->toRoute('personalloan',array( 
				'action' => 'calculate' 
		));  
		return array( 
				'form' => $form, 
	...
	protected function savepersonalloanAction() {
		$routeInfo = $this->getRouteInfo();
		$formValues = $this->params()->fromPost('formVal'); 
		$advancePaymentService = $this->getAdvancePaymentService(); 
		$advancePaymentService->savePersonalLoan($formValues,$routeInfo); 
		exit; 
	}
