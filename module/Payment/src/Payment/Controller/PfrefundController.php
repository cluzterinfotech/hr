...
use Application\Form\PfRefundForm;

class PfrefundController extends AbstractActionController
{
	...
	{   
		...
		$prg = $this->prg('/pfrefund/calculate',true); 
		...
		if ($form->isValid()) {
			...
			$this->redirect()->toRoute('pfrefund',array( 
				'action' => 'calculate' 
			));  
		} 
		return array(  
	...
	private function getForm() { 
        $form = new PfRefundForm(); 
        $company = $this->getServiceLocator()->get('company');
        $form->get('employeeIdPf')
	...
	public function closeAction() { 
		...
		$prg = $this->prg('/pfrefund/close', true);
		if ($prg instanceof Response ) {
			return $prg; 
		} elseif ($prg === false) {
			return array ('form' => $form); 
	...
	public function reportAction() {
		...
		if ($request->isPost()) { 
			...
			if ($form->isValid()) {
				return $this->redirect()->toRoute('pfrefund'); 
			}
		}
		return array(
				'form' => $form,
	...
	public function getpflastrefundAction() {
		$empId = $this->params()->fromPost('empNumber');
		$service = $this->getService(); 
		$res = $service->getLastPFRefDate($empId);
		echo json_encode($res); 
		exit; 
	}
	
	public function getpfdtlsAction() {
		$empId = $this->params()->fromPost('empNumber');
		$refundTo = $this->params()->fromPost('refTo'); 
		$refundFrom = $this->params()->fromPost('refFrom'); 
		$service = $this->getService(); 
		$res = $service->getPFDtls($empId,$refundFrom,$refundTo);
		echo json_encode($res);
		exit;
	}
	...
	private function getService() {
		return $this->getServiceLocator()->get('pfRefundService');
	} 
