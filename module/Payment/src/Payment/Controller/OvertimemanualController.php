...
use Payment\Form\OverTimeManualForm;
use Payment\Form\OvertimeManualFormValidator;
use Application\Model\OtManualEntryGrid;

class OvertimemanualController extends AbstractActionController { 
    ...
    public function addAction() { 
    	...
    	$prg = $this->prg('/overtimemanual/add', true);
    	...
    	if ($form->isValid()) {
    		...
    		$this->redirect()->toRoute('overtimemanual',array(
    				'action' => 'add'
    		)); 
    	}  
    ...
    public function removeAction()
    {
        ...
        $otService = $this->getService();
        $otService->removeManualOt($id);
        // echo json_encode($id);
        exit;
    }
    ...
    public function editAction() {
    	...
    	$prg = $this->prg('/overtimemanual/edit/'.$id, true);
    	if ($prg instanceof Response ) {
    		return $prg;
    	} elseif ($prg === false) {
    		return array ('form' => $form);
    ...
    public function getForm() {
        $form = new OverTimeManualForm(); 
    	$form->get('empIdOvertimeManual')
    	     ->setOptions(array('value_options' => $this->getEmployeeList()))
    	;
    	//$user = $this->getUser(); 
    	//$form->get('empIdOvertime')->setValue($user);
    ...
    public function approveAction() { 
    	...
    	$prg = $this->prg('/overtimemanual/approve/'.$id, true); 
    	if ($prg instanceof Response ) {
    		return $prg;
    	} elseif ($prg === false) {
    ...
    public function savemanualotAction() {
    	$formValues = $this->params()->fromPost('formVal'); 
    	$overtimeService = $this->getService();
    	$data = array( 
        	'empIdOvertime'       => $formValues['empIdOvertimeManual'],
        	...
        	//'startingDate'        => $formValues['empIdOvertime'],
        	//'endingDate'          => $formValues['empIdOvertime'],
        	'otStatus'            => '3',
    	); 
    	/*$data = array( 
        ...
        // \Zend\Debug\Debug::dump($data);  
        // exit;  
    	$overtimeService->saveManualOt($data,$formValues['month'],$formValues['year']);   
    	exit;   
    }    
    ...
    public function approvesupAction() {      
    	...
    	$form = $this->getApprovalForm();
    	$form->get('id')->setValue($id);
    	$prg = $this->prg('/overtimemanual/approvesup/'.$id, true);
    ...
    public function closeAction() { 
    	...
    	$prg = $this->prg('/overtimemanual/close', true);
    	if ($prg instanceof Response ) {
    		return $prg;
    	} elseif ($prg === false) {
    		return array ('form' => $form); 
    	...
    	$this->redirect()->toRoute('overtimemanual',array('action'=>'close')); 
    	return array(
    			'form' => $form,
    			$prg
    	);
    ...
    private function getFormValidator() {
        return new OvertimeManualFormValidator();
    }
